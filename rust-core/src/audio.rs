...
use oboe::{AudioOutputStreamSafe, AudioStreamBuilder, DataCallbackResult, Stereo};
...
pub fn init(config: &AudioConfig) -> EngineResult<()> {
    #[cfg(target_os = "android")]
    {
        ...
        let engine = AudioEngine { stream: Some(stream), buffer_queue: Vec::with_capacity(8), is_playing: true };
...
#[cfg(target_os = "android")]
fn create_oboe_stream(config: &AudioConfig) -> EngineResult<Box<dyn AudioOutputStreamSafe>> {
    use oboe::AudioOutputCallback;
    struct Cb { engine: &'static Mutex<AudioEngine> }
    impl AudioOutputCallback for Cb {
        type FrameType = (f32, Stereo);
        fn on_audio_ready(&mut self, _stream: &mut dyn AudioOutputStreamSafe, audio_data: &mut [(f32, f32)]) -> DataCallbackResult {
            if let Ok(mut eng) = self.engine.lock() {
                if !eng.is_playing { for s in audio_data.iter_mut() { *s = (0.0f32, 0.0f32); } return DataCallbackResult::Continue; }
                if let Some(frame) = eng.buffer_queue.first() {
                    let src: &[f32] = bytemuck::cast_slice(&frame.data);
                    let n = audio_data.len().min(src.len() / 2);
                    for i in 0..n { audio_data[i] = (src[i*2], src[i*2+1]); }
                    for s in &mut audio_data[n..] { *s = (0.0f32, 0.0f32); }
                    eng.buffer_queue.remove(0);
                } else { for s in audio_data.iter_mut() { *s = (0.0f32, 0.0f32); } }
            }
    ...
    let stream = AudioStreamBuilder::default()
        ...
        .set_callback(Cb { engine: engine_ref })
        .open_stream()
...
pub fn pause() { if let Some(e) = AUDIO_ENGINE.get() { if let Ok(mut eng) = e.lock() { eng.is_playing = false; if let Some(ref mut s) = eng.stream { let _ = s.pause(); } } } }
pub fn play() { if let Some(e) = AUDIO_ENGINE.get() { if let Ok(mut eng) = e.lock() { eng.is_playing = true; if let Some(ref mut s) = eng.stream { let _ = s.start(); } } } }
pub fn flush() { if let Some(e) = AUDIO_ENGINE.get() { if let Ok(mut eng) = e.lock() { eng.buffer_queue.clear(); if let Some(ref mut s) = eng.stream { let _ = s.stop(); let _ = s.start(); } } } }
