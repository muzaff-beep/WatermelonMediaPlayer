...
pub struct PlaylistEntry {
    /// Display title, if available (M3U EXTINF, PLS Title).
    pub title: Option<String>,
    /// URI or file path to the media.
    pub uri: String,
    /// Duration in seconds, if available (M3U EXTINF, PLS Length).
...
pub struct Playlist {
    ...
    /// Playlist title, if specified in metadata.
    pub title: Option<String>,
}
...
fn parse_m3u(path: &Path) -> EngineResult<Playlist> {
    let file = File::open(path).map_err(|e| {
        EngineError::Internal(format!("Failed to open M3U: {}", e))
    })?;
...
fn parse_m3u_lines(lines: &[String]) -> EngineResult<Playlist> {
    ...
    for line in lines {
        ...
        // EXTINF line: #EXTINF:<duration>,<title>
...
fn parse_pls(path: &Path) -> EngineResult<Playlist> {
    let file = File::open(path).map_err(|e| {
        EngineError::Internal(format!("Failed to open PLS: {}", e))
    })?;
    let reader = BufReader::new(file);
    let content: Vec<String> = reader.lines().filter_map(|l| l.ok()).collect();
...
fn parse_pls_lines(lines: &[String]) -> EngineResult<Playlist> {
    ...
    for line in lines {
        ...
        if lower.starts_with("title=") && !lower.starts_with("title") == false {
            // "Title=" (playlist title) vs "TitleN=" (entry title)
            let rest = &trimmed[6..];
        ...
        if lower.starts_with("numberofentries=") {
            let count_str = &trimmed[16..];
            entry_count = count_str.parse::<usize>().unwrap_or(0);
        ...
        if lower.starts_with("file") {
            ...
            if let Some(eq) = rest.find('=') {
                let _index: usize = rest[..eq].parse().unwrap_or(0);
                let uri = rest[eq + 1..].trim().to_owned();
                // Ensure entry exists at index-1
                let idx = _index.saturating_sub(1);
    ...
    // Remove entries with empty URIs (unresolved FileN)
...
mod tests {
    ...
    #[test]
    fn test_parse_pls_version_header_ignored() {
        let content = "[playlist]\nNumberOfEntries=1\nFile1=track.mp3\n";
        let playlist = parse_string(content, PlaylistFormat::PLS).unwrap();
        assert_eq!(playlist.entries.len(), 1);
    }
    ...
    #[test]
    fn test_format_detection_m3u() {
        let result = parse_file("test.m3u");
        assert!(result.is_err()); // File doesn't exist
    }

    #[test]
    fn test_format_detection_m3u8() {
        let result = parse_file("test.m3u8");
        assert!(result.is_err());
    }

    #[test]
    fn test_format_detection_pls() {
        let result = parse_file("test.pls");
        assert!(result.is_err());
    }
