...
#[no_mangle] pub extern "system" fn Java_com_watermelon_player_rust_WatermelonCore_nativeSetDataSource(mut env: JNIEnv, _: JClass, ptr: i64, uri: JString) -> u8 {
    if let Ok(s) = env.get_string(&uri) {
        engine_mut!(ptr).map(|e| e.set_data_source(&s.into()).is_ok() as u8).unwrap_or(0)
    } else { 0 }
}
...
#[no_mangle] pub extern "system" fn Java_com_watermelon_player_rust_WatermelonCore_nativeLoadSubtitle(mut env: JNIEnv, _: JClass, ptr: i64, path: JString) -> u8 {
    if let Ok(s) = env.get_string(&path) { engine_mut!(ptr).map(|e| e.load_subtitle(&s.into()).is_ok() as u8).unwrap_or(0) } else { 0 }
}
...
#[no_mangle] pub extern "system" fn Java_com_watermelon_player_rust_WatermelonCore_nativeSetSubtitleFont(mut env: JNIEnv, _: JClass, ptr: i64, path: JString) { if let Ok(s) = env.get_string(&path) { if let Some(e) = engine_mut!(ptr) { e.set_subtitle_font(&s.into()); } } }
#[no_mangle] pub extern "system" fn Java_com_watermelon_player_rust_WatermelonCore_nativeLoadPlugin(mut env: JNIEnv, _: JClass, ptr: i64, path: JString) -> u8 {
    if let Ok(s) = env.get_string(&path) { engine_mut!(ptr).map(|e| e.load_plugin(&s.into()).is_ok() as u8).unwrap_or(0) } else { 0 }
}
